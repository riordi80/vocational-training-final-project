...
**IoT**:
- Firmware ESP32 con sensores ambientales (SHT40, MH-Z19D, capacitivo suelo, LDR x2)
- Comunicación WiFi con backend vía HTTP REST
- Recopilación y envío automático de datos en tiempo real
...
**Tecnología**: React 19.2.0 + Vite 6 + Tailwind CSS
...
- Gestión de estado con React Hooks y Context API (AuthContext)
- Routing con React Router 7
- Cliente HTTP con Axios + interceptores JWT
- Despliegue automático en Vercel desde rama `main`

**Funcionalidades**:
- Sistema de autenticación real contra BD (Login/Register con JWT)
- Roles ADMIN y COORDINADOR con control de acceso por rutas
- Dashboard principal
- CRUD completo de árboles con formularios validados
- CRUD completo de centros educativos con mapa Leaflet
- CRUD completo de dispositivos ESP32 con configuración de umbrales
- Histórico de lecturas IoT con gráficas Recharts y tabla paginada
- Gestión de usuarios (solo ADMIN)

### 2.4 Aplicación Android

**Tecnología**: Java + Android SDK (API 24+ / target 36)
...
- Autenticación real contra BD con gestión de sesión en SharedPreferences
- `PermissionManager` centralizado para control de acceso por rol
- Consumo de API REST con Retrofit 2.9.0 + OkHttp 4.11.0
- Gráficas históricas con MPAndroidChart 3.1.0
- Polling de sensores en tiempo real con `Handler.postDelayed`

**Fragments principales**:
- Login, Register, Dashboard
- ListarCentros, DetalleCentro, FormularioCentro
- ArbolDetalles, CrearArbol
- HistoricoDispositivo, FormularioDispositivo
- AdminUsuarios, DetalleUsuario, FormularioUsuario
...
- SHT40 (I2C) — Temperatura y humedad ambiente
- Sensor capacitivo analógico (GPIO4) — Humedad del suelo
- MH-Z19D (UART) — CO₂ en ppm
- LDR x2 (GPIO5, GPIO35) — Niveles de luz

**Comunicación**:
- WiFi 2.4GHz con reintentos y reconexión automática
- Protocolo HTTP REST: `POST /api/lecturas` con payload JSON
- El dispositivo se identifica mediante su dirección MAC WiFi
- Frecuencia de lectura configurable desde BD (`frecuencia_lectura_seg`)
...
- El código backend está documentado con JavaDoc
- El sistema usa control de versiones Git con feature branches
- El sistema cuenta con documentación técnica completa
...
**Evidencia**: Código en `android/` con arquitectura Single-Activity + 13 fragments, landscape con `NavigationRailView`, histórico con MPAndroidChart
...
| BCryptPasswordEncoder | Incluida en Security | Hash de contraseñas |
| Spring Data JPA | 3.5.7 | ORM y repositorios |
| Hibernate | 6.6.x | Implementación JPA |
| Bean Validation | Incluida en Boot | Validación de entidades |
...
| Tecnología | Versión | Propósito |
|------------|---------|-----------|
| Android SDK | API 24 (min) / 36 (target) | Plataforma móvil |
...
| Tecnología | Versión | Propósito |
|------------|---------|-----------|
| Arduino Framework | — | Entorno de desarrollo ESP32 |
| ESP32 | — | Microcontrolador WiFi |
| ArduinoJson | — | Serialización JSON en C++ |
| SensirionI2CSht4x | — | Driver sensor SHT40 |
| Sensores SHT40, MH-Z19D, LDR | — | Temperatura, CO₂, luz |
...
### 7.3 Seguridad implementada — JWT + BCrypt

- **BCrypt**: las contraseñas se hashean en el registro (`passwordEncoder.encode`) y se verifican en el login (`passwordEncoder.matches`) mediante `BCryptPasswordEncoder` de Spring Security
- **JWT**: el login devuelve un token firmado (generado por `JwtService`); todas las peticiones protegidas pasan por `JwtAuthFilter` que valida el token antes de ejecutar el endpoint
- **Sesión stateless**: `SessionCreationPolicy.STATELESS` — el servidor no guarda estado de sesión

### 7.4 Consideraciones futuras

- Rate limiting en la API
- Auditoría de accesos (log de operaciones por usuario)
