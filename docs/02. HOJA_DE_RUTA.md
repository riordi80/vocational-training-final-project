...
**Estado**: [x] Fase 1-7 COMPLETADAS | [x] DESPLIEGUE COMPLETADO | [x] Fase 9 Testing COMPLETADA | [x] Fase 11 PUT N:M COMPLETADA | [x] Fase 14 CRUD entidades COMPLETADA | [x] Refactor Dispositivo-Centro COMPLETADO | [x] Fase 10 Android Landscape COMPLETADA | [x] Fase 13 Lighthouse COMPLETADA | [ ] Fase 12 Calidad parcial (12.1 y 12.2 pendientes) | [x] Fase 15 JWT+BCrypt COMPLETADA
...
- [x] Histórico de datos (temperatura/humedad) en Android — MPAndroidChart integrado en ArbolDetallesFragment (gráfica con selector DÍA/SEMANA/MES)
- [x] Layout landscape en Login y Registrer — landscape gestionado por NavigationRailView en activity_main (PR #275)
...
- [x] Ejercicio 3: Análisis Lighthouse
...
### Fase 10: Android — Histórico + Landscape [x]

- [x] **10.1** Histórico de datos [PGL]
  - [x] Gráfica MPAndroidChart integrada en `ArbolDetallesFragment` (no actividad separada)
  - [x] Selector de período DÍA / SEMANA / MES
  - [x] Conexión con `GET /api/lecturas/arbol/{id}/grafica?periodo=X`
  - [x] Datos: temperatura, humedad ambiente, humedad suelo

- [x] **10.2** Landscape [PGL]
  - [x] `res/layout-land/activity_main.xml` con `NavigationRailView` lateral (PR #275)
  - [x] Cubre todas las pantallas incluyendo Login y Register (arquitectura single-activity + fragments)

---

### Fase 11: Backend — PUT N:M [x]

- [x] **11.1** PUT en UsuarioCentroController [PGV]
  - [x] `PUT /api/usuario-centro/{id}` (actualizar `activo`, `usuarioId`, `centroId`)
  - [x] Validaciones (asignación existe, usuario es COORDINADOR, sin duplicados)
...
- [x] **12.3** Linting [DAD]
  - [x] `npm run lint` — 0 errores (PR #276)
  - [x] Globales Vitest añadidos al config, `coverage/` excluida, `CodeBlock` movido fuera del render

---

### Fase 13: SOJ — Análisis Lighthouse [x]

- [x] **13.1** Ejecutar Lighthouse en producción
  - [x] URL: https://vocational-training-final-project.vercel.app/
  - [x] Capturas de pantalla de los resultados (`docs/private/Requisitos por modulos/SOJ/`)

- [x] **13.2** Documento PDF con resultados
  - [x] `docs/private/Requisitos por modulos/SOJ/SOJ_Lighthouse_RicardoOrtiz_EnriquePerez.pdf`
...
### Fase 15: Seguridad — JWT + BCrypt [x]

> Mejora de 2ª evaluación (no es requisito formal de ningún módulo).

- [x] **15.1** BCrypt para contraseñas
  - [x] Añadir dependencia `spring-security-crypto` (o Spring Security completo)
  - [x] Hashear contraseñas en `POST /api/auth/register`
  - [x] Verificar hash en `POST /api/auth/login`

- [x] **15.2** JWT
  - [x] Añadir dependencia `jjwt`
  - [x] Generar token en login (payload: userId, email, rol)
  - [x] Filtro de validación de token en cada request protegido
  - [x] Actualizar frontend: enviar `Authorization: Bearer <token>` en las peticiones
  - [x] Actualizar Android: enviar cabecera `Authorization` en Retrofit
