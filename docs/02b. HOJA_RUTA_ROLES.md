...
- [ ] Crear `src/utils/permissions.js`
  - [ ] `hasGlobalPermission(user, permission)` - Verifica permisos globales
  - [ ] `hasRoleInCenter(user, centroId, roles)` - Verifica rol en centro específico
  - [ ] `canCreateArbol(user, centroId)` - Puede crear árbol en centro
  - [ ] `canEditArbol(user, centroId)` - Puede editar árbol en centro
  - [ ] `canDeleteArbol(user, centroId)` - Puede eliminar árbol en centro
  - [ ] `canManageCenter(user, centroId)` - Puede gestionar centro
  - [ ] `canAssignUsersToCenter(user, centroId)` - Puede asignar usuarios
  - [ ] `isAdmin(user)` - Es administrador global

#### 1.3 Crear hook personalizado
- [ ] Crear `src/hooks/usePermissions.js`
  - [ ] Hook que combina `useAuth()` con las utilidades de permisos
  - [ ] Retorna funciones de verificación listas para usar
...
- [ ] Modificar `src/context/AuthContext.jsx`
  - [ ] Usuario ahora incluye `centros` (array de asignaciones)
  ```javascript
  const user = {
    id: 1,
    nombre: 'Juan',
...
- [ ] Definir usuarios mock para cada combinación de roles
...
- [ ] Login reconoce usuarios de prueba
- [ ] Carga roles en centros del usuario

#### 2.4 Actualizar registro
- [ ] Registro crea usuarios con rol USUARIO
- [ ] Sin asignación a centros (debe ser asignado por ADMIN/COORDINADOR)
...
- [ ] Modificar `src/routes/ProtectedRoute.jsx`
  - [ ] Añadir prop `requiredRoles` (array de roles permitidos)
  - [ ] Añadir prop `requireCenterRole` (para rutas de centro específico)

#### 3.2 Crear componente RoleGate
- [ ] Crear `src/components/auth/RoleGate.jsx`
  - [ ] Componente que muestra/oculta children según permisos
  - [ ] Props: `roles`, `centroId`, `fallback`

#### 3.3 Crear página de acceso denegado
- [ ] Crear `src/pages/AccessDenied.jsx`
  - [ ] Mensaje claro de que no tiene permisos
  - [ ] Botón para volver al dashboard

#### 3.4 Actualizar rutas en App.jsx
- [ ] Añadir restricciones de rol a rutas sensibles
- [ ] Rutas de creación/edición requieren permisos
