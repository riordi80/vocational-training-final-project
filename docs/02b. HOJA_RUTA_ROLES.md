...
### Filtrado temporal en frontend (ELIMINAR con backend de roles)

Mientras el backend no filtra por permisos, las siguientes páginas filtran datos en JS (buscar comentarios `// TODO: eliminar cuando el backend filtre por permisos del token`):
- `ListadoArboles.jsx` - filtra centros y árboles accesibles
- `FormularioArbol.jsx` - filtra centros en el dropdown
- (las que se añadan en Fase 5 de centros)
