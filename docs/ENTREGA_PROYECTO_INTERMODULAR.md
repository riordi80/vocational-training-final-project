...
6. [Interfaces](#6-interfaces)
   - [6.1 Usabilidad y Accesibilidad](#61-usabilidad-y-accesibilidad)
...
Antes de escribir ningún componente de interfaz, el equipo realizó un proceso de diseño iterativo para anticipar problemas de usabilidad y accesibilidad:

1. **Prototipado en Figma**: Se crearon wireframes de las pantallas principales de la aplicación web (login, dashboard, listado de centros, detalle de árbol, histórico de dispositivo) en las issues #25 y #86. Esto permitió validar la jerarquía de navegación y el flujo de usuario antes de implementar ningún componente.

2. **Consideración de WCAG AA durante el diseño**: La paleta de colores de la aplicación web (verde oscuro `#2d6a4f` sobre blanco) fue seleccionada verificando que el contraste cumple el nivel AA del estándar WCAG 2.1 (ratio ≥ 4.5:1 para texto normal). Tailwind CSS facilita el uso coherente de estos colores mediante variables de clase.

3. **Revisión iterativa con feedback**: A lo largo del desarrollo, cada pantalla nueva fue revisada por ambos miembros del equipo antes de hacer merge a `develop`. Los criterios de revisión incluían: labels en todos los campos, mensajes de error claros, spinner en operaciones asíncronas y confirmación en acciones destructivas.

4. **Análisis Lighthouse post-desarrollo**: Una vez desplegada la aplicación en producción, se realizó un análisis con Google Lighthouse que evaluó rendimiento, accesibilidad, buenas prácticas y SEO. Los resultados fueron documentados y entregados en `SOJ_Lighthouse_RicardoOrtiz_EnriquePerez.pdf`.
...
Este manual describe el proceso completo que debe seguir el técnico responsable para desplegar el sistema **Proyecto Árboles** en la infraestructura de un nuevo cliente. Cubre la base de datos, el backend, el frontend, la aplicación Android y el firmware del dispositivo ESP32.

> **Requisitos previos**: cuenta en [Render](https://render.com) y en [Vercel](https://vercel.com), acceso al repositorio del proyecto y el archivo APK de la aplicación Android.

---

#### Paso 1 — Base de datos (PostgreSQL + TimescaleDB en Render)

1. En el panel de Render, crear un nuevo servicio de tipo **PostgreSQL**.
2. Anotar los datos de conexión generados: host interno, puerto, nombre de BD, usuario y contraseña.
3. Conectarse a la base de datos con cualquier cliente SQL (DBeaver, psql, etc.) y ejecutar el script `backend/create_database.sql` del repositorio para crear las 8 tablas y sus relaciones.
4. Habilitar la extensión TimescaleDB ejecutando:
   ```sql
...
#### Paso 2 — Backend (Spring Boot en Render)

1. En Render, crear un nuevo servicio de tipo **Web Service** apuntando al repositorio.
2. Configurar el **Build Command**: `./mvnw -pl backend clean package -DskipTests`
3. Configurar el **Start Command**: `java -jar backend/target/proyectoarboles-*.jar`
4. Añadir las siguientes **variables de entorno** en la configuración del servicio:

   | Variable | Valor |
   |---|---|
   | `SPRING_DATASOURCE_URL` | URL JDBC interna de Render (`jdbc:postgresql://...`) |
...
5. Desplegar el servicio y verificar que los logs muestran `Started ProyectoArbolesApplication` sin errores.
6. Anotar la URL pública del backend (p. ej. `https://proyecto-arboles-backend.onrender.com`).

---

#### Paso 3 — Frontend (React en Vercel)

1. En Vercel, importar el repositorio y configurar la carpeta raíz como `frontend`.
2. Vercel detecta automáticamente el framework (Vite). Verificar que el **Build Command** es `npm run build` y el **Output Directory** es `dist`.
3. Añadir la variable de entorno:

   | Variable | Valor |
   |---|---|
   | `VITE_API_URL` | URL pública del backend del paso anterior |

4. Desplegar. Vercel genera una URL pública para la aplicación web.
5. Acceder a la URL y verificar que el login funciona correctamente.

---

#### Paso 4 — Aplicación Android

1. Transferir el archivo **APK** al dispositivo Android del cliente (por cable USB, correo o enlace de descarga).
2. En el dispositivo, activar **"Instalar desde fuentes desconocidas"** en Ajustes → Seguridad (solo si el APK no se distribuye desde Play Store).
3. Abrir el APK y seguir el asistente de instalación.
4. Al iniciar la app por primera vez, introducir la URL del backend si la aplicación lo solicita en la pantalla de configuración.

> Si el cliente dispone de varios dispositivos Android, repetir los pasos 1–4 en cada uno.

---

#### Paso 5 — Firmware ESP32

Por cada dispositivo ESP32 que se vaya a instalar en el centro:

1. Abrir el archivo `esp32/config.h` (copiado desde `config.h.example`) y rellenar:
   ```cpp
   #define WIFI_SSID     "NombreRedWiFiDelCentro"
   #define WIFI_PASSWORD "ContraseñaWiFi"
   #define BACKEND_URL   "https://proyecto-arboles-backend.onrender.com"
   ```
2. Desde **Arduino IDE** o **PlatformIO**, compilar y subir el firmware al ESP32 mediante cable USB.
3. Abrir el monitor serie (115200 baudios) y verificar que el dispositivo se conecta al WiFi y envía lecturas al backend sin errores.
4. En la aplicación web o Android, registrar la **dirección MAC** del dispositivo y asignarlo al centro educativo correspondiente.

### 7.3 Manual de usuario: Aplicación Web

> **Versión**: 05-05-2026 · **Plataforma**: Web responsive · **Navegadores**: Chrome, Firefox, Safari, Edge (últimas versiones)
...
- `"Error al iniciar sesión"` → Backend en cold start o error de red; espera e intenta de nuevo
...
Abre el detalle del árbol → pulsa **"Editar"** → modifica los campos → pulsa **"Actualizar Árbol"**.
...
Solo ADMIN. En el listado, pulsa **"+ Añadir Centro"** → completa nombre, dirección, responsable, isla, población, provincia, código postal, teléfono, email y coordenadas (latitud/longitud) → pulsa **"Crear Centro"**.
...
Tabla con columnas: MAC, Estado (Activo/Inactivo), Frecuencia (segundos) y Última sincronización. Acciones por fila: **Ver Histórico** (todos), **Editar** (ADMIN o COORDINADOR asignado), **Eliminar** (solo ADMIN).
...
Solo ADMIN. En la tabla, pulsa **"Eliminar"** → confirma en el modal. El dispositivo deja de enviar lecturas.
...
![Captura: Botones de selección de período: Hoy, 7 días, 30 días, 6 meses, 1 año](img/frontend/captura-historico-filtros.png)
...
> **Versión**: 05-05-2026 · **Plataforma**: Android 7.0+ (API 24 o superior)
...
- Tabla de dispositivos instalados.
- Tabla de árboles plantados.
...
Ambas plataformas incluyen una pantalla de ayuda con el mismo contenido estructurado en ocho secciones expandibles: **Inicio de sesión y registro**, **Navegación**, **Dashboard**, **Centros educativos**, **Árboles**, **Dispositivos IoT**, **Alertas** y **Preguntas frecuentes**. La sección de preguntas frecuentes incluye específicamente la explicación del cold start de Render, los roles disponibles y los navegadores compatibles.

#### Aplicación Web

![Captura: Página de ayuda con acordeón de secciones](img/frontend/captura-ayuda.png)

La ayuda es accesible desde **dos puntos de entrada** sin necesidad de iniciar sesión:

- **Escritorio**: icono `?` (HelpCircle) en la barra de navegación superior, entre los links de navegación y el área de usuario. Se resalta en verde cuando la ruta `/ayuda` está activa.
- **Móvil**: enlace **«Ayuda»** en el menú hamburguesa (≡), al mismo nivel que Dashboard, Centros y Usuarios.

La ruta `/ayuda` está incluida dentro del `MainLayout` (con cabecera y pie de página) pero no requiere autenticación, lo que permite consultar la ayuda antes de registrarse o cuando el servidor está en cold start.

Cada sección es un **acordeón independiente**: se expande o colapsa al pulsar su cabecera. Varias secciones pueden estar abiertas simultáneamente.

#### Aplicación Android

![Captura: Fragment de ayuda con secciones expandibles](img/android/captura-ayuda.jpg)

La ayuda es accesible desde la navegación principal mediante el ítem **«Ayuda»** con el icono de interrogación:

- **Portrait**: ítem visible en el `BottomNavigationView`, entre Usuarios y Login/Logout.
- **Landscape**: ítem visible en el `NavigationRailView`. Para que los cinco ítems quepan en pantallas compactas, el rail usa modo **solo iconos** (`labelVisibilityMode="unlabeled"`) sin padding vertical entre ítems (`itemPaddingTop/Bottom="0dp"`).

El contenido se genera dinámicamente en `AyudaFragment` a partir de un array estático de secciones y preguntas, sin dependencias externas. Cada sección se infla como un `MaterialCardView` con cabecera verde clickable y contenedor colapsable; el icono chevron rota 180° con una animación de 200 ms al expandir.
...
| **Integración Spring** | ✓ Driver JDBC maduro | ✓ Muy maduro | ✓ Spring Data MongoDB | ! Limitada |
| **Nuestra elección** | ✓ | — | — | — |
...
|---|---|---|---|
| **Rendimiento** | ✓ Máximo | ✓ Alto | ! Medio-Alto |
| **Acceso a APIs del sistema** | ✓ Completo | ✓ Completo | ! Limitado |
...
| Aplicación Android | Ricardo/Enrique |
| Firmware ESP32 | Ricardo/Enrique |
